...
- **Synchronous Reject Notification Fix**: Swapped event emission ordering in [OrderPanel.ts](file:///d:/Devs/codetest-og/klinecharts-v10.0.0/src/ui/OrderPanel.ts) to resolve a race condition where the generic "Paper order submitted" notification immediately overwrote the synchronous "Order Rejected: Insufficient Margin" error message returned by the broker.
- **Live API Order Type Alignment**: Standardized live position exit payloads in [AccountManager.ts](file:///d:/Devs/codetest-og/klinecharts-v10.0.0/src/ui/AccountManager.ts) to send `quantity`, `price`, `trigger_price`, and `disclosed_quantity` as strings (e.g. `String(reqQty)`, `'0'`) to comply with strict OpenAlgo API schema validations and avoid potential `400 Bad Request` rejections.
- **Bracket Order Cancel ID Fallback Fix**: Updated the order ID mapping in the bracket order cancel routine inside [ChartManager.ts](file:///d:/Devs/codetest-og/klinecharts-v10.0.0/src/chart/ChartManager.ts) to resolve order ID using `order.orderid ?? order.order_id ?? order.id`. This prevents `undefined` values that occurred because of broker-specific field naming variations, resolving the `400 Bad Request` errors returned by OpenAlgo's strict schema validator.
- **Broker Name Badge Display**: Configured the execution engine badges in [AccountManager.ts](file:///d:/Devs/codetest-og/klinecharts-v10.0.0/src/ui/AccountManager.ts) and [OrderPanel.ts](file:///d:/Devs/codetest-og/klinecharts-v10.0.0/src/ui/OrderPanel.ts) to display the specific connected broker name (e.g., ZERODHA, UPSTOX, FYERS) instead of the generic "Live" or "LIVE" text when connected. The broker name is persisted to localStorage on connection pings and WebSocket auth successes and refreshed dynamically in the UI.
- **Online Status Badge Rendering Fix**: Fixed a state-desync bug where the connection status badge in [AccountManager.ts](file:///d:/Devs/codetest-og/klinecharts-v10.0.0/src/ui/AccountManager.ts) was initialized to hardcoded `'Offline'` upon every `render()` call. It now dynamically checks and matches the connection state returned by `wsClient.isAuth()` to prevent temporary UI status drops during manual/auto updates.
- **Account Manager UI Render Throttling**: Implemented a `250ms` throttle window for the sub-table rendering inside [AccountManager.ts](file:///d:/Devs/codetest-og/klinecharts-v10.0.0/src/ui/AccountManager.ts) during real-time tick updates (live and paper). Throttling prevents DOM table reconstructions 10-20 times per second, resolving high CPU loads, visual stutters, and user interaction freezes while preserving instant rendering (`immediate = true`) for manual actions like sub-tab clicks, query searching, settings adjustments, and order actions.
- **Paper Trading Account Reset Crash Fix**: Resolved a `TypeError: Cannot read properties of undefined (reading 'template')` crash inside [PaperBroker.ts](file:///d:/Devs/codetest-og/klinecharts-v10.0.0/src/paper/PaperBroker.ts) when submitting paper orders after a funds reset. The crash occurred because `resetAccount` re-assigned a clean state object without copying or restoring the active `chargeSettings` configuration. Updated `resetAccount()` to capture and preserve `chargeSettings` across resets, and moved the `carryCncPositionsToHoldings()` execution in the constructor to run after defaults are established to prevent initialization-order overrides.
...
## 2026-06-29d, and re-subscribe symbols.
